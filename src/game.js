...
class Game {
    constructor(elem, callback = noop) {
        this._init(elem, callback);

        this._createPlayStage();
    }
    ...
    _createPlayStage() {
        this._stages['play'] = this._engine.createStage();
        const playSize = {
            height: this._stages['play'].height,
            width: this._stages['play'].width
        }
        this._elem.appendChild(this._stages['play'].view);
        this._receptorContainer = this._stages['play'].createContainer((playSize.width - RECEPTOR_CONTAINER_WIDTH) / 2, 75);
        this._engine.AM.load([{
            name: 'receptor',
            path: 'assets/receptor.png'
        }], (resources) => {
            this._baseReceptors = this._setupReceptors([1, 1, 1, 1], this._receptorContainer);
            this._stages['play'].addChild(this._receptorContainer);
            this._callback();
        });
    }
    ...
    _loadSteps(location = '', callback = noop) {
        ...
        this._engine.AM.loadRemote(location, (err, response, body) => {
            this._stepContainer = this._stages['play'].createContainer(0, 75);
            ...
            this._steps.measures.forEach((measure, idx) => {
                ...
                let measureContainer = this._stages['play'].createContainer(0, 0);
                let offsetNote = this._stages['play'].height / numNotes;
                let currentOffsetY = 0;

                measureContainer.x = (this._stages['play'].width - 500) / 2;
                measureContainer.y = (this._stages['play'].height) * idx;
            ...
            this._stages['play'].addChild(this._stepContainer);
            callback();
        });
    }
    ...
    start() {
        ...
        var scene = this._stages['play'].scene;
        var frameOffset;
        var that = this;

        if (typeof steps.meta.DISPLAYBPM === 'string')
        ...
        function calculateFrameOffset() {
            ...
            var PixelsPerBeat = that._stages['play'].height / 4;
